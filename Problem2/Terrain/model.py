...
def train():
    img_4 = '/Users/sydne/CS3537/Problem2/tif_files/USGS_NED_1_n37w082_IMG.tif'
    # DownSized VERSION of 4th image
    # get data
    x_train, y_train = get_xy(img_4, 'ds')

    # create checkpoint that saves entropy in file name
    # output_path = '/Users/sydne/CS3537/Problem2/Img4_DS_output/'
    # save_freq = 1
    # model_checkpoint = setup_model_checkpoints(output_path, save_freq)

    # clear tensorflow computational graph
    ...
    # compile model with Entropy as a metric
    model.compile(loss='mean_squared_error', optimizer='adam', metrics=[Entropy(16)])

    # print the summary of the model
    mdoel.summary()

    # train the model with the data and using the checkpoint callback
    batch_size = 60
    epochs = 5
...
class Entropy(keras.metrics.Metric):
    ...
    def example():
        ...
        model = ...
        # compile model with Entropy as a metric
        model.compile(loss=loss, optimizer=optimizer, metrics=[Entropy()])
        # print the summary of the model
        model.summary()
        # train the model with the data and using the checkpoint callback
        model.fit(x, y, batch_size=batch_size, epochs=epochs, verbose=1,
                  callbacks=[model_checkpoint])
        # load one of the checkpointed models, for example:
        model2 = load_model('problem_2_output/model_params/model.00010_9.8867.h5')
        # estimate the elevation for i=0, j=0
        print(model2.predict(np.array([[0, 0]], dtype='float32')))
